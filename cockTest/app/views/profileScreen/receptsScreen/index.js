    ...
    return (
      <ScrollView contentContainerStyle={{flexWrap:'wrap',flexDirection:'row',alignItems:'flex-start' , justifyContent:'space-between' , marginStart:10, marginEnd:10}} style={{backgroundColor:colors.silver}}>
        {favData?.map((item , index) => (
          <View key={index} style={{width:'45%',height:300,marginTop:10}}>
            <RecipeBlock  key={index} publisher={item.publisher} imageUrl={item.imageUrl} title={item.title} recipeId={item.recipeId} fav={item.isFavorite} />
        </View>
        ))}
      </ScrollView>
    )
